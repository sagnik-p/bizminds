...
, ["_id","date_sold"])
    ...
    prompt_parts = [
        'analyse the given data and tell me which product made the maximum profit for mechant with merchant id ',str(merchantId),'    ', str(modified_json_objects), ' Just send the product id only'
    ]
...
, ["_id","shop_name","email","phone","address","product_prices"])
    ...
    prompt_parts = [
        'analyse the given data and tell me if there is any anomaly . Data of information about suppliers:  ',str(modified_suppliers),'    and data of information of merchant purchases from suppliers: ',str(modified_purchases),' and the data of product sales of the merchant:  ',str(modified_sales_info), '     if possible anomaly is found, return the information about that anomalous purchase  . If no anomaly found, return NOT_FOUND'
    ]
    return model.generate_content(prompt_parts).text
